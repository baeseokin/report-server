...
CREATE TABLE `approval_requests` (
  ...
  `approval_status` enum('대기','승인','반려') DEFAULT '대기' COMMENT '결재 상태',
  PRIMARY KEY (`id`)
...
CREATE TABLE `approval_items` (
  ...
  CONSTRAINT `approval_items_ibfk_1` FOREIGN KEY (`request_id`) REFERENCES `approval_requests` (`id`) ON DELETE CASCADE
